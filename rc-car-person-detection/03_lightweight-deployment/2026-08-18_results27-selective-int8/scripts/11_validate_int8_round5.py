...
def main():
    ...
    candidates = sorted(models.glob("results27_640_int8_round5_*.onnx"))
    # Include the strong Round 4 reference candidate if present.
    round4_suffix = models / "results27_640_int8_round4_q3_q4_suffix50.onnx"
    model_paths = [fp32]
    if round4_suffix.exists():
        model_paths.append(round4_suffix)
    model_paths.extend(candidates)

    if not fp32.exists():
        raise FileNotFoundError(fp32)
    if not candidates:
        raise FileNotFoundError("No Round 5 candidates. Run 10_quantize_int8_round5.py first.")
    ...
    for p in model_paths:
        ...
        sessions[p.stem] = ort.InferenceSession(
    ...
    csv_path = results / "int8_round5_final_mix.csv"
    ...
    print()
    print("Decision:")
    print("- Prefer >=95% retain across 0.15~0.25.")
    print("- Compare against Round4 q3+q4 candidate, which is already a strong mixed-precision baseline.")
    print("- Final deployment decision requires Raspberry Pi latency + real school-field FP/FN.")
